...
{
	assert_spin_locked(&current->sighand->seccomplock);
...
static inline void seccomp_assign_mode(struct task_struct *task,
				       ...
				       unsigned long flags)
{
	assert_spin_locked(&task->sighand->seccomplock);
...
void seccomp_filter_release(struct task_struct *tsk)
{
	...
	/* We are effectively holding the seccomplock by not having any sighand. */
 ...
 *
 * Caller must be holding current->sighand->seccomplock lock.
 *
...
static bool has_duplicate_listener(struct seccomp_filter *new_child)
{
	...
	lockdep_assert_held(&current->sighand->seccomplock);
					     ...
					     unsigned long filter_off)
{
	 ...
	 */
	spin_lock_irq(&task->sighand->seccomplock);

	if (task->seccomp.mode != SECCOMP_MODE_FILTER) {
		spin_unlock_irq(&task->sighand->seccomplock);
		return ERR_PTR(-EINVAL);
	}
