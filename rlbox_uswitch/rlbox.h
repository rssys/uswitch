...
namespace rlbox
{
	...
	public:
		...
		template <typename T, typename U, typename ... TArgs, RLBOX_ENABLE_IF(my_is_void_v<return_argument<T>> && sandbox_function_have_all_args_fundamental_or_wrapped<TArgs...>::value && my_is_invocable_v<T, sandbox_removeWrapper_t<TArgs>...>)>
		void invokeWithFunctionPointerWithExtraData(T* fnPtr, U data, TArgs&&... params)
		{
			// TODO: use std::forward?
			this->impl_InvokeFunctionWithExtraData(fnPtr, data, sandbox_removeWrapper(this, params)...);
		}

		template <typename T, typename U, typename ... TArgs, RLBOX_ENABLE_IF(!my_is_void_v<return_argument<T>> && sandbox_function_have_all_args_fundamental_or_wrapped<TArgs...>::value && my_is_invocable_v<T, sandbox_removeWrapper_t<TArgs>...>)>
		tainted<return_argument<T>, TSandbox> invokeWithFunctionPointerWithExtraData(T* fnPtr, U data, TArgs&&... params)
		{
			// TODO: use std::forward?
			tainted<return_argument<T>, TSandbox> ret = sandbox_convertToUnverified<return_argument<T>>(this, this->impl_InvokeFunctionWithExtraData(fnPtr, data, sandbox_removeWrapper(this, params)...));
			return ret;
		}
